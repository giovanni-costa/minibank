  ...
  async create(data: CreateTransferDto, password: string) {
    ...
    if (data.date) {
      throw new HttpException(
        'Dates are autogenerated',
        HttpStatus.BAD_REQUEST,
      );
    }

    // Throws Error if sender or receiver Name is giver
    if (data.senderName || data.receiverName) {
      throw new HttpException(
        'Sender and Receiver Names are Server-Side',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    await this.prisma.miniBanco.update({
      data: {
        amount:
          senderData.amount.toNumber() - parseFloat(data.value.toString()),
      },
      where: {
        id: senderData.id,
      },
    ...
    await this.prisma.miniBanco.update({
      data: {
        amount:
          receiverData.amount.toNumber() + parseFloat(data.value.toString()),
      },
      where: {
        id: receiverData.id,
      },
    ...
    const create = await this.prisma.transfers.create({
      data: {
        ...data,
        ...{ senderName: senderData.name, receiverName: receiverData.name },
      },
    });
